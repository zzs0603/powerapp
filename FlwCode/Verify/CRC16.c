#include "CRC16.h"
//==================================================================//
//CRC表
//==================================================================//
static const char CRC_TABLE[]=
{
    
};



//============================================//
//===========CRC计算函数（查表法）============//
void crc16(char Data,CRCCODE *Code)
{
    int temp;
    temp = Code -> Crcl;
    temp = Data ^ temp;
    temp=temp<<1;
    Code -> Crcl = CRC_TABLE[temp + 1] ^ Code -> Crch;
    Code -> Crch = CRC_TABLE[temp];
}


void crc16block(const char *pData, int Size, CRCCODE *Code)
{
    int i;
    const char *pChar;
    static CRCCODE tmpCode;

    if (Code == (void*)0)
    {
        return;
    }

    if (pData == (void*)0) 
    {
        Code -> Crcl = 0xff;
        Code -> Crch = 0xff;
        return;
    }

    tmpCode = *Code;
    
    for (i = 0, pChar = pData; i < Size; i++, pChar++)
    {
        char Data = *pChar;
        int temp = tmpCode.Crcl;
        temp = Data ^ temp;
        temp = temp << 1;
        tmpCode.Crcl = CRC_TABLE[temp + 1] ^ tmpCode.Crch;
        tmpCode.Crch = CRC_TABLE[temp];
    }

    *Code = tmpCode;
}

