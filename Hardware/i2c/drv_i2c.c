...
void I2C_ER_Handler(void)
{
                       ...
                       I2C_SR1_BERR ))                                    // If AF, BERR or ARLO, abandon the current job and commence new if there are jobs
    {
        ...
        if (!(SR1Register & I2C_SR1_ARLO) && !(I2Cx->CR1 & I2C_CR1_STOP)) // If we dont have an ARLO error, ensure sending of a stop
        {
            ...
            {  
                int32_t err_overflow = I2C_DEFAULT_TIMEOUT;
                ...
                err_overflow = I2C_DEFAULT_TIMEOUT;
...
void I2C_EV_Handler(void)
{
    ...
    else if (SReg_1 & I2C_SR1_BTF)                                      // Byte transfer finished - EV7_2, EV7_3 or EV8_2
    {
        ...
        wait_overflow = I2C_DEFAULT_TIMEOUT;
        while (I2Cx->CR1 & I2C_CR1_START) { if(--wait_overflow <= 0) break; }                         // We must wait for the start to clear, otherwise we get constant BTF
    }
...
uint8_t i2cWriteBuffer(I2C_TypeDef *I2C, uint8_t addr_, uint8_t reg_, uint8_t len_, uint8_t *data)
{
    uint8_t i;
    uint8_t my_data[16];
    ...
    if (!(I2Cx->CR2 & I2C_IT_EVT))                                      // If we are restarting the driver
    {
        ...
        {
            int32_t wait_overflow = I2C_DEFAULT_TIMEOUT;
            while (I2Cx->CR1 & I2C_CR1_STOP) { if(--wait_overflow <= 0) break; }                      // Wait for any stop to finish sending
...
uint8_t i2cWrite(I2C_TypeDef *I2C, uint8_t addr_, uint8_t reg_, uint8_t data)
{
...
uint8_t i2cRead(I2C_TypeDef *I2C, uint8_t addr_, uint8_t reg_, uint8_t len, uint8_t *buf)
{
    uint32_t timeout = I2C_DEFAULT_TIMEOUT;
    ...
    if (I2C == I2C1)
    {
        ...
        for (i = 0; i < 8; i++)
        {
            while (!GPIO_ReadInputDataBit(I2C1_GPIO, I2C1_SCL_PIN))  // Wait for any clock stretching to finish
               delay_ms(3);                                            // 2.5 would be 400 kHz, 3 is 333.33333 kHz
...
void i2c_reset_errcount(I2C_TypeDef *I2C)
{
    if (I2C == I2C1)
         i2c1ErrorCount = 0;
    else
